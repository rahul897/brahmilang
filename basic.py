...
class Lexer:
    ...
    def make_identifier(self):
        ...
        if id_str in CODE_ENDS and self.fn!='<stdin>':
            return Token(TT_EOF, id_str, pos_start, self.pos)
...
class Parser:
    def __init__(self, tokens, fn='<stdin>'):
        self.fn = fn
        self.tokens = tokens
        self.tok_idx = -1
        self.advance()
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'nannu_involve_cheyakandi'):
            res.register_advancement()
            self.advance()
            return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))

        if self.current_tok.matches(TT_KEYWORD, 'aapandroi'):
            res.register_advancement()
            self.advance()
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'bokka_ji'):
            res.register_advancement()
            self.advance()
    ...
    def comp_expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected int, float, identifier, '+', '-', '(', '[', 'idi_ok_antaventra', 'FOR', 'nuv_line_lo_undu', 'FUN' or 'NOT'"
            ))
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'idi_ok_antaventra'):
        ...
        elif tok.matches(TT_KEYWORD, 'nuv_line_lo_undu'):
    ...
    def if_expr_b(self):
        return self.if_expr_cases('poni')

    def if_expr_c(self):
        res = ParseResult()
        else_case = None

        if self.current_tok.matches(TT_KEYWORD, 'ok_anadanta_edava'):
            res.register_advancement()
            self.advance()
            ...
            if self.current_tok.type == TT_NEWLINE:
                ...
                if self.current_tok.matches(TT_KEYWORD, 'idi_code_a_vellehe'):
                    res.register_advancement()
                    self.advance()
                else:
                    return res.failure(InvalidSyntaxError(
    ...
    def if_expr_b_or_c(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'poni'):
    ...
    def while_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'nuv_line_lo_undu'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'nuv_line_lo_undu'"
            ))
...
BuiltInFunction.print = BuiltInFunction("aa_chupentra_idi_chudu")
...
global_symbol_table.set("aa_chupentra_idi_chudu", BuiltInFunction.print)
